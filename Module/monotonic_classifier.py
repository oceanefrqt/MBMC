...
def index_leaves(A, k):
    ...
    lev = k - p_lev
    #return {i: (len(A) - lev) + i for i in range(1, lev+1)} | {lev+i: (len(A) - k) + i for i in range(1, p_lev+1)}
    ind_leaves = {}
...
def compute_model(data, z, rev, up):
    ...
    for i in range(len(X)):
        ...
        if up:
            err0, err1 = err(lab, w, z[0]), err(lab, w, z[1])
        else:
            err0, err1 = err(lab, w, z[1]), err(lab, w, z[0])
...
def compute_recursion(data, z, case=None):
    ...
    if case is None:
        for rev in [True, False]:
            for up in [True, False]:
                ...
                models[case_num] = compute_model(data, z,rev, up)
    else:
